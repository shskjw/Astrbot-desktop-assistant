...
class DesktopClientApp(QObject):
    ...
    def _schedule_reconnect(self):
        ...
        if max_attempts > 0 and self._reconnect_attempts >= max_attempts:
            ...
            if self._floating_ball:
                self._floating_ball.show_bubble(f"连接失败，已达最大重试次数")
            return
    ...
    def _handle_output_message(self, message: OutputMessage):
        ...
        # 检查免打扰模式
        do_not_disturb = self.config.interaction.do_not_disturb
        
        # 检查是否正在等待响应（用户主动发起的对话）
        is_user_waiting = self._floating_ball and self._floating_ball.is_waiting_response()
        ...
        if msg_type == "text":
            ...
            if is_proactive_response or should_silent:
                if message.streaming:
                    # 流式响应时累积内容
                    if not hasattr(self, '_silent_response_buffer'):
            ...
            if is_proactive_response or should_silent:
                ...
                buffer = getattr(self, '_silent_response_buffer', '')
                if buffer:
                    get_chat_history_manager().add_message(
                        role="assistant",
                        content=buffer,
    ...
    def _on_settings_changed(self, settings: dict):
        ...
        if 'do_not_disturb' in interaction:
            self.config.interaction.do_not_disturb = interaction['do_not_disturb']
