...
class WebSocketClient:
    ...
    def __init__(
        ...
        self.on_command = (
            on_command  # 命令处理回调: (command, request_id, params) -> result
        )
        ...
        self._connection_state: str = (
            "disconnected"  # disconnected, connecting, connected, reconnecting
        )
    ...
    def _set_connection_state(self, state: str):
        ...
        if self._connection_state != state:
            ...
            logger.debug(f"连接状态: {old_state} -> {state}")
    ...
    async def _connect_loop(self):
        ...
        while self._running:
            try:
                ...
                # 安全地显示连接 URL（隐藏 token）
                safe_url = self.url.split("?")[0] if "?" in self.url else self.url
                if reconnect_attempts > 0:
                    logger.info(f"正在重连 (第 {reconnect_attempts} 次): {safe_url}")
                else:
                    logger.info(f"正在连接 WebSocket: {safe_url}")
                ...
                async with websockets.connect(
                    ...
                    if is_reconnect:
                        ...
                        logger.info(
                            f"✅ 重连成功（第 {self._total_reconnects} 次重连）"
                        )
                        # 触发重连成功回调
                        ...
                        try:
                            ...
                            if data.get("type") == "heartbeat_ack":
                                ...
                                if latency > self.HIGH_LATENCY_THRESHOLD:
                                    ...
                                    logger.warning(
                                        f"⚠️ 心跳延迟过高: {latency:.2f}s ({self._high_latency_count}/{self.MAX_HIGH_LATENCY_COUNT})"
                                    )
                                else:
                            ...
                            if data.get("type") == "server_config":
                                ...
                                logger.debug(
                                    f"收到服务端配置: {self._server_timeout_config}"
                                )
                                continue
                            ...
                            if data.get("type") == "connection_status":
                                ...
                                logger.debug(f"服务端确认连接状态: {status}")
                                if config:
                                    self._server_timeout_config = config
                                    logger.debug(f"服务端配置: {config}")
                                continue
                            ...
                            if data.get("type") == "busy_state_ack":
                                ...
                                logger.debug(
                                    f"忙碌状态确认: is_busy={is_busy}, operation={operation}"
                                )
                                continue
                ...
                if e.code == 1000:
                    ...
                    logger.error("❌ 连接异常断开（网络问题）")
                else:
                    logger.warning(f"连接关闭: code={e.code}, reason={e.reason}")

            except websockets.InvalidMessage as e:
                # WebSocket 握手失败 - 提供详细诊断
                logger.error(f"❌ WebSocket 握手失败: {e}")
                logger.error("可能的原因:")
                logger.error("  1. 服务端未运行或未启动 WebSocket 服务")
                logger.error("  2. 连接到了错误的 URL 或端口")
                # 显示 URL（隐藏 token）
                safe_url = self.url.split("?")[0] if "?" in self.url else self.url
                logger.error(f"  3. 尝试连接的 URL: {safe_url}")
                logger.error("  4. 防火墙或代理拦截")
                logger.error("  5. 服务端可能不支持 WebSocket 协议")
                logger.error("建议:")
                logger.error("  - 确认 AstrBot 服务端已启动")
                logger.error("  - 检查服务端是否启用了 desktop_assistant 插件")
                logger.error("  - 检查服务端日志是否有 WebSocket 相关错误")
                logger.error("  - 验证配置的服务器地址和端口是否正确")
                ...
                safe_url = self.url.split("?")[0] if "?" in self.url else self.url
                logger.error(f"  - 请检查服务端是否运行在 {safe_url}")
                logger.error("  - 确认端口未被其他程序占用")
            except asyncio.TimeoutError:
                logger.error("❌ 连接超时")
                logger.error("  - 检查网络连接")
                logger.error("  - 确认服务器地址可访问")
            except OSError as e:
                # 网络不可达等系统级错误
                logger.error(f"❌ 网络错误: {e}")
                logger.error("  - 检查网络连接状态")
                logger.error("  - 确认服务器地址和端口正确")
            except Exception as e:
        ...
        while self._running and self.ws:
            try:
                ...
                if time_since_last_ack > self.HEARTBEAT_ACK_TIMEOUT:
                    ...
                    logger.warning(
                        f"⚠️ 心跳响应超时 ({self._heartbeat_failures}/{self._max_heartbeat_failures})，距上次响应 {time_since_last_ack:.1f}s"
                    )

                    # 连续失败达到阈值才断开
                    if self._heartbeat_failures >= self._max_heartbeat_failures:
                        logger.debug("❌ 心跳失败次数过多，主动断开连接以触发重连")
                ...
                if self._high_latency_count >= self.MAX_HIGH_LATENCY_COUNT:
                    logger.warning(
                        f"⚠️ 连续 {self._high_latency_count} 次高延迟，连接质量差，主动重连"
                    )
                ...
                if time_since_pong > self.PING_INTERVAL * 2:
                    logger.warning(
                        f"⚠️ 底层 pong 超时: {time_since_pong:.1f}s，可能存在网络问题"
                    )

            except asyncio.CancelledError:
                break
            except websockets.ConnectionClosed:
                logger.debug("心跳时连接已关闭")
                break
            except Exception as e:
                consecutive_send_failures += 1
                logger.warning(f"⚠️ 心跳发送失败 ({consecutive_send_failures}/3): {e}")

                # 连续发送失败 3 次，认为连接已断开
                if consecutive_send_failures >= 3:
                    logger.debug("❌ 心跳发送连续失败，断开连接")
                    break
    ...
    async def _handle_command(self, data: dict):
        ...
        if not command:
            logger.debug("收到无效命令: 缺少 command 字段")
            return

        logger.debug(f"收到命令: {command}, request_id={request_id}")
        ...
        if self.on_command:
            try:
                ...
                await self.send_command_result(
                    command, request_id, {"success": False, "error_message": str(e)}
                )
        else:
            logger.debug(f"未设置命令处理器，忽略命令: {command}")
    ...
    async def set_busy_state(
        self, is_busy: bool, operation: str = "", duration: int = 30
    ):
        """
        设置忙碌状态
        ...
        if is_busy:
            logger.debug(f"进入忙碌状态: {operation}，预计 {duration}s")
        else:
            logger.debug(f"退出忙碌状态: {operation}")
    ...
    def get_connection_stats(self) -> dict:
        ...
        return {
            ...
            "last_message_age": current_time - self._last_message_time
            if self._last_message_time
            else None,
            "last_heartbeat_age": current_time - self._last_heartbeat_ack
            if self._last_heartbeat_ack
            else None,
            "last_pong_age": current_time - self._last_pong_time
...
class AstrBotApiClient:
    ...
    def is_fully_connected(self) -> bool:
        ...
        return (
            ...
            and self._ws_connection_state == "connected"
        )
        ...
        while True:
            try:
                ...
                if not is_http_healthy:
                    ...
                    if ws_connected:
                        logger.debug("HTTP 检测失败但 WebSocket 仍连接，暂不断开")
                    ...
                    if self._health_check_failures >= effective_max_failures:
                        ...
                        if ws_connected:
                            ...
                            logger.warning(
                                "⚠️ HTTP 健康检测失败但 WebSocket 正常，保持连接"
                            )
                            self._health_check_failures = 0  # 重置计数，继续监控
                        else:
                            logger.debug("❌ HTTP 和 WebSocket 都异常，触发重连")
                            self._health_check_failures = 0
                            self.state = ConnectionState.DISCONNECTED
                else:
                    # 检测成功，重置失败计数
    ...
    def _on_ws_connection_state_change(self, state: str):
        ...
        logger.debug(f"WebSocket 状态变化: {old_state} -> {state}")
    ...
    def _on_ws_reconnect(self):
        ...
        """
        logger.debug("WebSocket 重连成功，重置 HTTP 失败计数")
        self._health_check_failures = 0
        self._last_successful_request = time.time()
    ...
    async def get_sessions(
        self, platform_id: Optional[str] = "webchat"
    ) -> tuple[bool, Any]:
        """
        获取会话列表
    ...
    async def send_message(
        ...
        try:
            ...
            if isinstance(e, (httpx.ConnectTimeout, httpx.PoolTimeout)):
                ...
                yield SSEEvent(
                    event_type="error", data=f"请求超时（服务器响应时间过长）: {e}"
                )
